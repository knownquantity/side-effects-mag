...
async function latest() {
  ...
  divider()
  out(dim(`read an issue: ${accent('npx side-fx read <id>')}`))
  blank()
}
...
async function read(id) {
  if (!id) {
    ...
    out(dim('usage: npx side-fx read <id>'))
    blank()
    process.exit(1)
  }
  ...
  if (!article) {
    ...
    out(dim('try: npx side-fx latest'))
    blank()
    process.exit(1)
  }
...
switch (cmd) {
  ...
  default:
    ...
    out(dim('run `npx side-fx help` for available commands'))
    blank()
    process.exit(1)
}
